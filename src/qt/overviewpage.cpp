...
// Copyright (c) 2018-2019 The PointOfPublicCoin developers
...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::POPC)
    {
    }
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount popcAvailableBalance = balance - immatureBalance - nLockedBalance;

    // POPC Watch-Only Balance
    CAmount nTotalWatchBalance = watchOnlyBalance + watchUnconfBalance;
    CAmount nAvailableWatchBalance = watchOnlyBalance - watchImmatureBalance - nWatchOnlyLockedBalance;

    // POPC labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, popcAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // POPC Available
    bool showPOPCAvailable = settingShowAllBalances || popcAvailableBalance != nTotalBalance;
    bool showWatchOnlyPOPCAvailable = showPOPCAvailable || nAvailableWatchBalance != nTotalWatchBalance;
    ui->labelBalanceText->setVisible(showPOPCAvailable || showWatchOnlyPOPCAvailable);
    ui->labelBalance->setVisible(showPOPCAvailable || showWatchOnlyPOPCAvailable);
    ui->labelWatchAvailable->setVisible(showPOPCAvailable && showWatchOnly);

    // POPC Pending
    bool showPOPCPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyPOPCPending = showPOPCPending || watchUnconfBalance != 0;
    ui->labelPendingText->setVisible(showPOPCPending || showWatchOnlyPOPCPending);
    ui->labelUnconfirmed->setVisible(showPOPCPending || showWatchOnlyPOPCPending);
    ui->labelWatchPending->setVisible(showPOPCPending && showWatchOnly);
    ...
    // POPC Locked
    bool showPOPCLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyPOPCLocked = showPOPCLocked || nWatchOnlyLockedBalance != 0;
    ui->labelLockedBalanceText->setVisible(showPOPCLocked || showWatchOnlyPOPCLocked);
    ui->labelLockedBalance->setVisible(showPOPCLocked || showWatchOnlyPOPCLocked);
    ui->labelWatchLocked->setVisible(showPOPCLocked && showWatchOnly);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("POPC")
    updateDisplayUnit();
}
...
void OverviewPage::SetLinks()
{
    ...
    ui->labelLinksUrl4->setText("<a href=\"https://twitter.com/pointofpubliccoin\">https://twitter.com/pointofpubliccoin</a>");
