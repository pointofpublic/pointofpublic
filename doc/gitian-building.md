...
*Setup instructions for a gitian build of PointOfPublicCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the PointOfPublicCoin
...
- [Building PointOfPublicCoin](#building-pointofpubliccoin)
...
Clone the git repositories for pointofpubliccoin and gitian and then checkout the pointofpubliccoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the pointofpubliccoin repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building PointOfPublicCoin
----------------------

To build PointOfPublicCoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the pointofpubliccoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/pointofpubliccoin/.git/
...
For example:
```bash
URL=https://github.com/konez2k/pointofpubliccoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit pointofpubliccoin=${COMMIT} --url pointofpubliccoin=${URL} ../pointofpubliccoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit pointofpubliccoin=${COMMIT} --url pointofpubliccoin=${URL} ../pointofpubliccoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit pointofpubliccoin=${COMMIT} --url pointofpubliccoin=${URL} ../pointofpubliccoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/pointofpubliccoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/pointofpubliccoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/pointofpubliccoin-build.assert
```
...
[pointofpubliccoin/gitian.sigs](https://github.com/PointOfPublicCoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
